...
class GeographyType(AtomicType):
    ...
    DEFAULT_GEOGRAPHY_SRID = 4326

    def __init__(self, srid: int = None):
        if srid is None:
            self.srid = GeographyType.DEFAULT_GEOGRAPHY_SRID
        elif srid != GeographyType.DEFAULT_GEOGRAPHY_SRID:
    ...
    def simpleString(self) -> str:
        if self.srid == GeographyType.DEFAULT_GEOGRAPHY_SRID:
            return "geography"
        else:
